...
import { Message } from './EditTaskModalStyling';
import Modal from "react-modal";
...
const EditTaskModal = ({isOpen, onRequestClose, onEditTask, task}) =>
{
  ...
  const [taskDueDate, setTaskDueDate] = useState(task.time);
  ...
  const handleEditTask = async event => 
  {
    ...
    // Check for any empty fields
    if (!editTaskName.value) {setMessage("* Please enter a task name *"); return;}
    if (!taskDueDate) {setMessage("* Please select a task due date *"); return;}
    if (!taskCategory) {setMessage("* Please select a task category *"); return;}

    // Update the task object with the new information
    let updatedTask = {...task, taskContent: editTaskName.value, time: taskDueDate, category: taskCategory, user:user.user}
    let obj = updatedTask, js = JSON.stringify(obj);
    ...
    try
    {
      ...
      if (res.error.length > 0) {setMessage(res.error);}
      else {setMessage(<span id="successMessagePopUp">* Task has been edited *</span>);}
  ...
  return (
    <div>
      ...
      <Modal isOpen={isOpen} onRequestClose={onRequestClose} style={customStyles}>
        <text id="createAList">EDIT TASK</text>
        <button id="closePopUp" className="modal-close" onClick={onRequestClose}>X</button>
        <div>
          <input id="tasknameField" type="text" class="form-control col-md-12" placeholder="TASK NAME" ref={(c) => (editTaskName = c)} defaultValue={task.taskContent}/>
        </div>
        <div>
          <input id="taskDueDate" type="datetime-local" required onChange={(e) => setTaskDueDate(e.target.value)}></input>
        </div>
